...
const brief = (matrix$1, {
  ...
  const [h, w] = (_matrix = matrix$1, matrix.size(_matrix));
  if (!h || !w) return util.AEU;
  const {
    raw,
    text
  } = mattro(matrix$1, {
  ...
  const pads = columnsIndicator.Max(len(preset || ansi))(text);
  const dyes = preset && fluoMatrix.fluo(raw, {
    direct,
    preset,
    colorant: true
  }); // const columns = zipperBand(text, pads, (col, pad) =>mapper(col,x=>npad(x,)))

  const lines = preci.padMx(text, raw, dyes, pads, ansi).map(line => `[${line}]`);
  return '[' + lines.join(`,${util.RN} `) + ']';
};
