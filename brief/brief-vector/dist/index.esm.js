import { AEU, maxLen, rpad, lpad } from '@spare/util';
import { vettro } from '@spare/vettro';
import { brief as brief$1 } from '@spare/brief-entries';
import { fluoVector } from '@palett/fluo-vector';
import { FRESH as FRESH$1, JUNGLE } from '@palett/presets';
...
const brief = (arr, {
  ...
  if (indexed) return brief$1(Object.entries(arr), {
    abstract,
    head,
    tail,
    preset,
...
const FRESH = {
  ...
  na: Palett.brown.base
};
const SUBTLE = {
  max: Palett.grey.lighten_5,
  min: Palett.grey.darken_1,
  na: Palett.indigo.lighten_2
};
const GLACIAL = {
  max: Palett.teal.accent_2,
  min: Palett.blue.darken_3,
  na: Palett.brown.accent_3
};
const METRO = {
...
const padEnds = (arr, {
  ...
  return Array.isArray(pad = (_pad = pad) !== null && _pad !== void 0 ? _pad : maxLen(arr, ansi)) ? arr.map((x, i) => rpad(x, pad[i], ansi, fill)) : arr.map(x => rpad(x, pad, ansi, fill));
};
/**
 *
 * @param {string[]} arr
...
const padStarts = (arr, {
  ...
  return Array.isArray(pad = (_pad2 = pad) !== null && _pad2 !== void 0 ? _pad2 : maxLen(arr, ansi)) ? arr.map((x, i) => lpad(x, pad[i], ansi, fill)) : arr.map(x => lpad(x, pad, ansi, fill));
};

export { brief, hBrief, padEnds, padStarts, vBrief };
