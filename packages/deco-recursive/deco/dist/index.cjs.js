...
var nullish$1 = require('@typen/nullish');
...
const presetDeco = p => {
  ...
  if (nullish$1.nullish(p.presets)) p.presets = (_p$pr = p.pr) !== null && _p$pr !== void 0 ? _p$pr : [presets.AZURE, presets.MOSS];
  DecoConfig.prototype.assignPresets.call(p, presets.AZURE, presets.MOSS);
  if (nullish$1.nullish(p.depth)) p.depth = 8; // 展示级别

  if (nullish$1.nullish(p.vert)) p.vert = 0; // 在此级别以下均设为竖排

  if (nullish$1.nullish(p.unit)) p.unit = 32; // 若 数组/键值对的值 单个元素长度超过此, 则进行竖排

  if (nullish$1.nullish(p.width)) p.width = 80; // 字符超过此, 则换行

  if (nullish$1.nullish(p.string)) p.string = {};
  const s = p.string; // if (nullish(s.presets)) s.presets = [ATLAS, SUBTLE]

  DecoConfig.prototype.assignPresets.call(s, presets.ATLAS, presets.SUBTLE); // p |> JSON.stringify |> logger
