...
var Mapper$1 = require('@vect/vector-mapper');
...
const transpose$4 = (mx, h, w) => {
  ...
  for (--w; w >= 0; w--) cols[w] = Mapper$1.mapper(mx, r => r[w], h);
...
const column = (mx, c, h) => Mapper$1.mapper(mx, r => r[c], h);

const columns$1 = function (y, h) {
  return Mapper$1.mapper(this, r => r[y], h);
};
...
const lookupIndexes$1 = function (labels) {
  return Mapper$1.mapper.call(this, labels, lookupIndex$1);
};
/**
 *
 * @param {str|[*,*]} [label]
...
const selectSamples$1 = function (fieldIndexPairs) {
  ...
  return Mapper$1.mapper(rows, row => {
    let o = {};
    Mapper$1.iterate(fieldIndexPairs, ([field, index]) => o[field] = row[index], depth);
    return o;
  });
};
/**
...
const selectSamples = function (fieldIndexPairs) {
  const {
    rows
        ...
        depth = fieldIndexPairs === null || fieldIndexPairs === void 0 ? void 0 : fieldIndexPairs.length;
  return Mapper$1.mapper(columns, column => {
    let o = {};
    Mapper$1.iterate(fieldIndexPairs, ([field, index]) => o[field] = column[index], depth);
    return o;
  });
};
/**
...
const lookupIndexes = function (labels) {
  return Mapper$1.mapper.call(this, labels, lookupIndex);
};
/**
 *
 * @param {str|[*,*]} [label]
...
const pop = matrix => Mapper$1.mapper(matrix, row => row.pop());
/**
...
const shift = matrix => Mapper$1.mapper(matrix, row => row.shift());
/**
...
class CrosTab {
  ...
  setRowBy(r, fn) {
    return Mapper$1.mutate(this.row(r), fn, this.width), this;
  }
  ...
  mapSide(fn, {
    ...
    return this.boot({
      side: Mapper$1.mapper(this.side, fn)
    }, mutate);
  }
  ...
  mapBanner(fn, {
    ...
    return this.boot({
      head: Mapper$1.mapper(this.head, fn)
    }, mutate);
  }
  ...
  mutateSide(fn) {
    return Mapper$1.mutate(this.side, fn), this;
  }

  mutateBanner(fn) {
    return Mapper$1.mutate(this.head, fn), this;
  }
  ...
  sort({
    ...
    if (direct === ROWWISE$1) sortKeyedRows.call(o, comparer$1, this.coin(field));
    if (direct === COLUMNWISE$1) sortTabular.call(o, comparer$1, this.roin(field));
    return mutate ? this : this.copy(o);
  }
  ...
  sortByLabels({
    ...
    if (direct === ROWWISE$1) sortRowsByKeys$1.call(o, comparer$1);
    if (direct === COLUMNWISE$1) sortTabularByKeys$1.call(o, comparer$1);
    return mutate ? this : this.copy(o);
  }
...
fluoVector.fluoVector.bind(MUTATE_PIGMENT$1);
...
function palettCrostab({
  ...
  if (dyed) {
    ...
    space === HEX ? crostab.mutate(hex => {
      ...
      return _mapper = Mapper$1.mapper(xyz, v => v.toFixed(0).padStart(3)), dyeFactory(xyz)(_mapper);
    });
  }
...
const sortBy = function (indicator, comparer) {
  const vec = this,
        kvs = Mapper$1.mutate(vec, (x, i) => [indicator(x, i), x]).sort(toKeyComparer(comparer));
  return Mapper$1.mutate(kvs, ([, value]) => value);
};
...
function* presetFlopper({
  ...
  const defaultPreset = presets.randPreset(defaultColor);
...
class Pal extends Callable$1 {
  ...
  br(words) {
    return this.des += enumChars.SP + bracket$2.parenth(words), this;
  }

  to(someone) {
    if (someone instanceof Pal) someone = someone.name;
    this.des += ' -> ' + bracket$2.bracket(someone);
    return this;
  }
...
class Says {
  ...
  aboard(name, presets) {
    ...
    return _classPrivateFieldGet(this, _roster)[name] = Pal.build(DecoStringEnv.deco(String(name), {
      presets,
      effects
    }));
  }
...
const boundToLeap = bound => {
  ...
  if (!nullish.nullish(min)) {
    if (!nullish.nullish(dif)) return {
      min,
      dif
    };
    if (!nullish.nullish(max)) return {
      min,
      dif: max - min
    };
  }

  if (!nullish.nullish(dif)) {
    if (!nullish.nullish(max)) return {
      min: max - dif,
      dif
    };
...
class ProjectorFactory {
  ...
  constructor(leap, preset, effects) {
    if (nullish.nullish(preset)) return new VoidProjectorFactory();
  ...
  static build(bound, {
    ...
    preset = preset ? (_preset = preset, presets.presetToLeap(_preset)) : null;
...
const scale = (x, min$1, lever, base, ceil) => comparer.min((comparer.max(x, min$1) - min$1) * lever + base, ceil);
...
function fluoByRows(mx, config) {
  const context = this,
        mapper$1 = context !== null && context !== void 0 && context.mutate ? Mapper$1.mutate : Mapper$1.mapper;
...
class PointColorFactory {
  static color([[bX, pX], [bY, pY]]) {
    ...
    return (_, i, j) => {
      ...
      if (!nullish.nullish(v = bX && bX[i][j])) {
        var _pX$color;

        return _pX$color = pX.color(v), toColor(_pX$color);
      }

      if (!nullish.nullish(v = bY && bY[i][j])) {
        var _pY$color;
...
const renderEntries = function (entries, lv) {
  ...
  let pad;
  const rows = (lv < vert || entries.some(([, v]) => lange.lange(v) > unit) || !width) && (pad = (_entries = entries, mutateKeyPad(_entries))) ? Mapper$1.mutate(entries, ([k, v]) => lpad(k, pad) + enumChars.RTSP + v) : wrapEntries(entries, width);
...
const renderString = function (string, level, indent) {
  ...
  return DecoStringEnv._decoString.call({
...
function prettyNode(node, level = 0, indent) {
  ...
  if (t === enumDataTypes.FUN) return level >= this.depth ? DecoFuncEnv.funcName(node) : DecoFuncEnv.decoFunc(node, this);
  ...
  if (t === enumDataTypes.OBJ) {
    ...
    const {
      depth
          ...
          pt = typ.typ(node);
    if (pt === enumObjectTypes.ARRAY) return level >= depth ? '[array]' : (_deVe$call2 = deVe.call(this, node.slice(), level), bracket$2.bracket(_deVe$call2));
    if (pt === enumObjectTypes.OBJECT) return level >= depth ? '{object}' : (_deEn$call3 = deEn.call(this, Object.entries(node), level), bracket$2.brace(_deEn$call3));
    if (pt === enumObjectTypes.DATE) return level >= depth ? formatDate.formatDate(node) : formatDateTime.formatDateTime(node);
    if (pt === enumObjectTypes.MAP) return level >= depth ? '(map)' : (_deEn$call4 = deEn.call(this, [...node.entries()], level), bracket$2.bracket(_deEn$call4));
    if (pt === enumObjectTypes.SET) return level >= depth ? '(set)' : `set:[${deVe.call(this, [...node], level)}]`;
    return `${node}`;
  }
...
const deVe = function (vector, lv) {
  const config = this;
  Mapper$1.mutate(vector, v => String(_deco.call(config, v, lv + 1)));
  if (config.fluos) fluoVector.fluoVector.call(MUTATE_PIGMENT$1, vector, config.fluos);
...
const deEn = function (entries, lv) {
  ...
  if (config.fluos) fluoEntries.fluoEntries.call(MUTATE_PIGMENT$1, entries, config.fluos);
...
const presetDeco = p => {
  ...
  fluo.FluoConfigs.prototype.assignPresets.call(s.fluos, presets.ATLAS, presets.SUBTLE);
