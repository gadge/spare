...
import { Deco as InternalDeco }   from './src/Deco.js'
...
const node = InternalDeco.prototype.node
...
export const Deco = (p = {}) => {
  ...
  return node.bind(new InternalDeco(p))
}
...
export const deco = (o, p = {}) => {
  ...
  p.broad = p.broad ?? false   // 宽幅展示; set if broaden view
  p.indent = p.indent ?? 0
  return node.call(new InternalDeco(p), o, p.indent)
}
...
export const decoPlain = (o, p = {}) => {
  ...
  p.width = p.width ?? 80     // 换行宽度; linefeed if line width exceeds 'width'
  p.broad = p.broad ?? false  // 宽幅展示; set if broaden view
  return node.call(new InternalDeco(p), o)
}
