...
var presetDeco$1 = require('@spare/preset-deco');
var oneself$3 = require('@ject/oneself');
...
function Dye$2(color) {
  if (!color) return oneself$3.oneself;
...
const lambdafy = (text, pretty, defaultName = 'anonym') => {
  if (!THIS_REG.test(text)) text = pretty ? text.replace(FUNCTION_BODY, (_, name, args, body) => nameDye(name === 'anonymous' ? defaultName : name) + enumChars.SP + bracket$2.parenth(argsDye(args.trim())) + enumChars.SP + arrowDye('=>') + bodyDye(body)) : text.replace(FUNCTION_BODY, (_, name, args, body) => name + enumChars.SP + bracket$2.parenth(args) + enumChars.SP + '=>' + body);
...
function palettCrostab({
  ...
  if (space !== HEX$1) {
    crostab.mutate(space === RGB$1 ? hexToRgb$1 : space === HSL$3 ? hexToHsl$1 : oneself$3.oneself);
  }
...
const presetToFlat = preset => {
  ...
  if (!preset) return oneself$3.oneself;
  const na = preset.na;
...
const renderEntries = function (entries, lv) {
  ...
  let pad;
  const rows = (lv < vert || entries.some(([, v]) => lange(v) > unit) || !width) && (pad = (_entries = entries, mutateKeyPad(_entries))) ? mutate(entries, ([k, v]) => lpad(k, pad) + enumChars.RTSP + v) : wrapEntries(entries, width);
...
const wrapEntries = function (entries, width) {
  ...
  iterate$1(entries, ([k, v]) => {
    ...
    len += lange(kvp = k + enumChars.RTSP + v) + sp;
  ...
  if (t === OBJ) {
    ...
    const {
      depth
          ...
          pt = typ(node);
    if (pt === ARRAY) return level >= depth ? '[array]' : (_deVe$call2 = deVe.call(this, node.slice(), level), bracket$2.bracket(_deVe$call2));
    if (pt === OBJECT) return level >= depth ? '{object}' : (_deEn$call3 = deEn.call(this, Object.entries(node), level), bracket$2.brace(_deEn$call3));
    if (pt === DATE) return level >= depth ? formatDate(node) : formatDateTime(node);
    if (pt === MAP) return level >= depth ? '(map)' : (_deEn$call4 = deEn.call(this, [...node.entries()], level), bracket$2.bracket(_deEn$call4));
    if (pt === SET) return level >= depth ? '(set)' : `set:[${deVe.call(this, [...node], level)}]`;
    return `${node}`;
  }
  ...
  if (config.presets) {
    ...
    if (config.full) {
      if (Array.isArray(config.presets)) {
        ...
        presets[0].filter = x => isNumeric$2(x) || charsetFullwidth.isNumeric(x);

        presets[0].mapper = charsetFullwidth.parseNum;
      } else {
        presets = Object.assign({}, config.presets);

        presets.filter = x => isNumeric$2(x) || charsetFullwidth.isNumeric(x);

        presets.mapper = charsetFullwidth.parseNum;
      }
    }
...
const cosmetics = function (rows = []) {
  ...
  const br = (_Br = bracket$2.Br(bracket)) !== null && _Br !== void 0 ? _Br : oneself$3.oneself;
...
const Deco = (p = {}) => cosmetics.bind(presetDeco$1.presetMatrix(p));
/***
 *
 * @param {*[][]} matrix
 * @param {Object} p
...
const deco = (matrix, p = {}) => cosmetics.call(presetDeco$1.presetMatrix(p), matrix);
