...
const _decoEntries = function (entries = []) {
  ...
  const config = this;
  if (!((_entries = entries) != null && _entries.length)) return liner([], config);
  let {
    ansi,
    dash,
    delim,
...
const deco = (entries, p = {}) => _decoEntries.call(DecoConfig.parse(p, CONFIG, DUAL_PRESET_COLLECTION), entries);
