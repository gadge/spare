...
const lambdafy = (text, pretty, defaultName = 'anonym') => {
  if (!THIS_REG.test(text)) text = pretty ? text.replace(FUNCTION_BODY, (_, name, args, body) => nameDye(name === 'anonymous' ? defaultName : name) + SP$h + bracket.parenth(argsDye(args.trim())) + SP$h + arrowDye('=>') + bodyDye(body)) : text.replace(FUNCTION_BODY, (_, name, args, body) => name + SP$h + bracket.parenth(args) + SP$h + '=>' + body);
...
const _decoEntries = function (entries = []) {
  ...
  bracket$1 = (_Br = bracket.Br(bracket$1)) !== null && _Br !== void 0 ? _Br : oneself$3.oneself;
