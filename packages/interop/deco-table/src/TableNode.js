import { Grad, Node }                     from '@spare/node'
import { DA, LF, SP, VO }                 from '@texting/enum-chars'
import { COLUMNWISE, POINTWISE, ROWWISE } from '@vect/enum-matrix-directions'
import { width }                          from '@vect/matrix-index'
import { init }                           from '@vect/vector-init'
import { Concat }                         from './Concat.js'

const VB = ' | ', CX = '-+-'

export class TableNode {
  #knd
  #vnd
  constructor(conf) {
    this.#knd = new Node(conf, conf.keys)
    this.#vnd = new Node(conf)
  }

  table(table, direct, indent = 0) {
    if (direct === POINTWISE) return Concat.stand(this.pointwiseLines(table), LF, indent)
    if (direct === ROWWISE) return Concat.stand(this.rowwiseLines(table), LF, indent)
    if (direct === COLUMNWISE) return Concat.stand(this.columnwiseLines(table), LF, indent)
    return Concat.stand(this.pointwiseLines(table), LF, indent)
  }

  * headLines(tvs, nvs, wds, grad, sgr) {
    let j, line
    const kn = this.#knd, wd = tvs.length
    line = sgr ? (SP.repeat(sgr.wd) + VB) : VO
    for (j = 0; j < wd; j++) line += (j ? VB : VO) + kn.render(grad, tvs[j], nvs[j], wds[j])
    yield line
    line = sgr ? (DA.repeat(sgr.wd) + CX) : VO
    for (j = 0; j < wd; j++) line += (j ? CX : VO) + DA.repeat(wds[j])
    yield line
  }

  * pointwiseLines({ head, side, rows }) {
    const kn = this.#knd, vn = this.#vnd
    const ht = rows?.length ?? 0, wd = head?.length ?? width(rows), cn = ht * wd
    const vts = Array(cn), vns = Array(cn), sts = Array(ht), sns = Array(ht), hts = Array(wd), hns = Array(wd)
    const vgr = new Grad(vn.uns), hgr = head ? new Grad(kn.uns) : null, sgr = side ? new Grad(kn.uns) : null
    const xs = Array(ht).fill(0), ys = Array(wd).fill(0)
    let sw = 0, hw = 0, w = 0, p = 0
    if (side) for (let i = 0; i < ht; i++) { if ((xs[i] = kn.store(sgr, sts, sns, side[i], i)) > sw) sw = xs[i] }
    if (head) for (let j = 0; j < wd; j++) { if ((ys[j] = kn.store(hgr, hts, hns, head[j], j)) > hw) hw = ys[j] }
    for (let i = 0; i < ht; i++) {
      for (let j = 0, row = rows[i]; j < wd; j++) {
        (w = vn.store(vgr, vts, vns, row[j], p++)), (w > xs[i]) && (xs[i] = w), (w > ys[j]) && (ys[j] = w)
      }
    }
    hgr?.lever(kn.presm, hw), sgr?.lever(kn.presm, sw)
    if (head) { yield* this.headLines(hts, hns, ys, hgr, sgr) }
    let i, j, line
    for (i = 0, p = 0; i < ht; i++) {
      line = side ? (kn.render(sgr, sts[i], sns[i], sgr.wd) + VB) : VO
      for (j = 0, vgr.lever(vn.presm, xs[i]); j < wd; j++, p++) line += (j ? VB : VO) + vn.render(vgr, vts[p], vns[p], ys[j])
      yield line
    }
  }

  * rowwiseLines({ head, side, rows }) {
    const kn = this.#knd, vn = this.#vnd
    const ht = rows.length, wd = head?.length ?? width(rows), cn = ht * wd
    const vts = Array(cn), vns = Array(cn), sts = Array(ht), sns = Array(ht), hts = Array(wd), hns = Array(wd)
    const vgr = init(ht, () => new Grad(vn.uns)), hgr = new Grad(kn.uns), sgr = new Grad(kn.uns)
    const xs = Array(ht).fill(0), ys = Array(wd).fill(0)
    let sw = 0, hw = 0, w = 0, p = 0
    if (side) for (let i = 0; i < ht; i++) { if ((xs[i] = kn.store(sgr, sts, sns, side[i], i)) > sw) sw = xs[i] }
    if (head) for (let j = 0; j < wd; j++) { if ((ys[j] = kn.store(hgr, hts, hns, head[j], j)) > hw) hw = ys[j] }
    for (let i = 0; i < ht; i++) {
      for (let j = 0, row = rows[i]; j < wd; j++) {
        (w = vn.store(vgr[i], vts, vns, row[j], p++)), (w > xs[i]) && (xs[i] = w), (w > ys[j]) && (ys[j] = w)
      }
    }
    hgr?.lever(kn.presm, hw), sgr?.lever(kn.presm, sw)
    if (head) { yield* this.headLines(hts, hns, ys, hgr, sgr) }
    let i, j, line
    for (i = 0, p = 0; i < ht; i++) {
      line = side ? (kn.render(sgr, sts[i], sns[i], sgr.wd) + VB) : VO
      for (j = 0, vgr[i].lever(vn.presm, xs[i]); j < wd; j++, p++) line += (j ? VB : VO) + vn.render(vgr[i], vts[p], vns[p], ys[j])
      yield line
    }
  }

  * columnwiseLines({ head, side, rows }) {
    const kn = this.#knd, vn = this.#vnd
    const ht = rows.length, wd = head?.length ?? width(rows), cn = ht * wd
    const vts = Array(cn), vns = Array(cn), sts = Array(ht), sns = Array(ht), hts = Array(wd), hns = Array(wd)
    const vgr = init(wd, () => new Grad(vn.uns)), hgr = new Grad(kn.uns), sgr = new Grad(kn.uns)
    const xs = Array(ht).fill(0), ys = Array(wd).fill(0)
    let sw = 0, hw = 0, w = 0, p = 0
    if (side) for (let i = 0; i < ht; i++) { if ((xs[i] = kn.store(sgr, sts, sns, side[i], i)) > sw) sw = xs[i] }
    if (head) for (let j = 0; j < wd; j++) { if ((ys[j] = kn.store(hgr, hts, hns, head[j], j)) > hw) hw = ys[j] }
    for (let i = 0; i < ht; i++) {
      for (let j = 0, row = rows[i]; j < wd; j++) {
        (w = vn.store(vgr[j], vts, vns, row[j], p++)), (w > xs[i]) && (xs[i] = w), (w > ys[j]) && (ys[j] = w)
      }
    }
    hgr?.lever(kn.presm, hw), sgr?.lever(kn.presm, sw)
    if (head) { yield* this.headLines(hts, hns, ys, hgr, sgr) }
    let i, j, line
    for (j = 0; j < wd; j++) { vgr[j].lever(vn.presm, ys[j]) }
    for (i = 0, p = 0; i < ht; i++) {
      line = side ? (kn.render(sgr, sts[i], sns[i], sgr.wd) + VB) : VO
      for (j = 0; j < wd; j++, p++) line += (j ? VB : VO) + vn.render(vgr[j], vts[p], vns[p], ys[j])
      yield line
    }
  }
}



