...
const FluoNumber = (bound, preset = PLANET, colorant = true) => {
  ...
  let dye;
  return vleap.dif && cleap.dif.some(n => !!n) ? (dye = BlendDye(vleap, cleap), colorant ? x => numStrict.isNumeric(x) ? dye(x) : prime : x => {
    var _x, _x2;

    return numStrict.isNumeric(x) ? (_x = x, dye(x)(_x)) : (_x2 = x, prime(_x2));
  }) : (dye = (_cleap$min = cleap.min, hslToDye(_cleap$min)), colorant ? x => numStrict.isNumeric(x) ? dye : prime : x => {
    var _x3, _x4;
