...
export function keepBracket(tx) {
  return tx => hasBr(tx) ? tx : bracket(tx)
}

export function keepParenth(tx) {
  return tx => hasBr(tx) ? tx : parenth(tx)
}

export function keepSnakePrettyKey(tx) {
  return tx => hasBr(tx) ? tx : tx => bracket(Rosters.main.get(ansiOrSnake(tx)))
}
