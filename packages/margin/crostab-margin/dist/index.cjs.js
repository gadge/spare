...
const marginCopy = (mx, t, b, l, r, h, w) => {
  ...
  for (let i = 0; i < t; i++) x[i] = vectorMargin$1.marginCopy(mx[i], l, r, w);

  for (let i = bs; i < h; i++) x[i] = vectorMargin$1.marginCopy(mx[i], l, r, w);
...
class MatrixMargin {
  ...
  emptyRow(el) {
    ...
    return vectorMargin$1.marginMapper(Array(width), () => el, left, right, width);
  }
