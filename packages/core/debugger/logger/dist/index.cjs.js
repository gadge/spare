...
const Qt = mode => {
  if (mode === APOS || mode === enumChars.QT) return quote;
  if (mode === DITTO || mode === enumChars.DT) return ditto;
  return null;
...
const lambdafy = (text, pretty) => {
  if (!THIS_REG.test(text)) text = pretty ? text.replace(LAMB_REG, (_, name, args, body) => nameDye(name) + enumChars.SP + parenth(argsDye(args)) + enumChars.SP + arrowDye('=>') + bodyDye(body)) : text.replace(LAMB_REG, (_, name, args, body) => name + enumChars.SP + parenth(args) + enumChars.SP + '=>' + body);
...
function deco(node) {
  ...
  if (t === OBJ) {
    ...
    const pt = typ(node);
    if (pt === ARRAY) return _node$map$join = node.map(deco.bind(this)).join(enumChars.COSP), bracket(_node$map$join);
    if (pt === OBJECT) return _mutate$map$join = mutate(Object.entries(node), decoKey, deco.bind(this)).map(pairEnt).join(enumChars.COSP), brace(_mutate$map$join);
...
exports.DecoFlat = DecoFlat$1;
exports.DecoFunc = DecoFunc$1;
