...
export function deco (node) {
  ...
  if (t === OBJ) {
    const pt = typ(node)
    if (pt === ARRAY) return node.map(deco.bind(this)).join(COSP) |> bracket
    if (pt === OBJECT) return mutate(Object.entries(node), decoKey, deco.bind(this)).map(pairEnt).join(COSP) |> brace
