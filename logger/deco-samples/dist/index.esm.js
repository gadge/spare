import { FRESH as FRESH$1, JUNGLE, SUBTLE as SUBTLE$1 } from '@palett/presets';
...
const cosmati = function (samples) {
  ...
  let [pick, head] = fields ? (_lookupKeys$call = lookupKeys.call(sample, fields), unwind(_lookupKeys$call)) : [keys.slice(), keys.slice()];
  const {
    head: l,
    tail: r,
    dash: dashY
  ...
  if (preset) dye = preset && fluoMatrix(raw, {
    direct,
    preset,
    stringPreset,
    colorant: true
