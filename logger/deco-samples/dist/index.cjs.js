...
const cosmati = function (samples) {
  ...
  if (!samples.length) return util.AEU;
  ...
  let [pick, head] = fields ? (_lookupKeys$call = objectSelect.lookupKeys.call(sample, fields), entriesUnwind.unwind(_lookupKeys$call)) : [keys.slice(), keys.slice()];
  const {
    head: l,
    tail: r,
    dash: dashY
  ...
  pick = new vettro.Vectogin(pick, l, r, dashY).toVector();
  head = new vettro.Vectogin(head, l, r, dashY).toVector();
  rows = new vettro.Vectogin(samples, t, b, dashX).map(sample => objectSelect.selectValues(sample, pick)).toVector();
  let [h, w] = matrix.size(rows);
  const {
    raw,
    text
  } = mattro.mattro(rows, {
  ...
  if (preset) dye = preset && fluoMatrix.fluoMatrix(raw, {
    direct,
    preset,
    stringPreset,
    colorant: true
