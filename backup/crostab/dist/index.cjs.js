...
class CrosTabX {
  ...
  static brief(crosTab, {
    ...
    side = preci.Preci.fromArr(side, _s.head, _s.tail).stringify(_s.abstract).toList('..');
    banner = preci.Preci.fromArr(banner, _b.head, _b.tail).stringify(_b.abstract).toList('..');
    const {
      rawx,
      palx,
      wordx
