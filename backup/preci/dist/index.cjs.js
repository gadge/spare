...
const destructPreci = (arr, [h, t], {
  ...
  let preci = Preci.fromArr(arr, h, t),
      raws = preci.toList('...'),
      pals = (_visual = visual, util.isVisual(_visual)) ? hatsuMatrix.Visual.vector(raws, { ...visual,
    retFn: true,
    mutate: false
  }) : null,
...
const destructPreX = (mx, [top, bottom], [left, right], {
  ...
  const prex = PreX.fromMx(mx, [top, bottom], [left, right], [height, width]),
        rawx = prex.toMx('..'),
        palx = (_visual = visual, util.isVisual(_visual)) ? hatsuMatrix.Visual.matrix(rawx, { ...visual,
    retFn: true,
    mutate: false
  }) : null,
...
const padMx = (wordx, rawx, palx, pads, ansi) => {
  ...
  return palx ? wordx.map((words, i) => util.vecPalPad(words, rawx[i], palx[i], pads, ansi)) : wordx.map((words, i) => util.vecPad(words, rawx[i], pads, ansi));
};
...
const padTable = (head, wordx, rawx, palx, ansi = false, chinese = false) => {
  ...
  const pads = veho.Mx.columns([head].concat(wordx), col => util.maxLen(col, ansi)),
        [_head, blanc, rows] = [veho.Ar.zip(head, pads, (x, p) => util.lpad(x, p, ansi)), pads.map(l => '-'.repeat(l)), padMx(wordx, rawx, palx, pads, ansi)];
  return {
    head: _head,
    blanc,
    rows
...
const padTableCn = (head, wordx, rawx, palx, ansi = false) => {
  ...
  const pads = veho.Mx.columns([head].concat(wordx), col => ({
    pd: util.maxLen(col, ansi),
    cn: col.some(string.hasChn)
  })),
        [_head, blanc, rows] = [veho.Ar.zip(head, pads, (x, {
    cn,
    pd
  }) => cn ? util.rpad(string.toFullAngle(x), pd, ansi, space) : util.rpad(x, pd, ansi)), pads.map(p => (p.cn ? dash : '-').repeat(p.pd)), palx ? wordx.map((words, i) => veho.Ar.zip(words, pads, (tx, {
    cn,
    pd
  }, j) => {
    var _numPad, _numPad2;

    return cn ? (_numPad = util.numPad(string.toFullAngle(tx), rawx[i][j], pd, ansi, space), palx[i][j](_numPad)) : (_numPad2 = util.numPad(tx, rawx[i][j], pd, ansi), palx[i][j](_numPad2));
  })) : wordx.map((words, i) => veho.Ar.zip(words, pads, (tx, {
    cn,
    pd
  }, j) => cn ? util.numPad(string.toFullAngle(tx), rawx[i][j], pd, ansi, space) : util.numPad(tx, rawx[i][j], pd, ansi)))];
  return {
    head: _head,
    blanc,
    rows
...
exports.PreX = PreX;
exports.Preci = Preci;
exports.destructPreX = destructPreX;
exports.destructPreci = destructPreci;
exports.padMx = padMx;
